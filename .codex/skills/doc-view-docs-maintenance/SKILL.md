...
3. Update the most specific doc for the topic: architecture, roadmap, contract design, performance, compatibility, workflow, MCP, i18n, or
   release.
4. If adding a repo-local skill, make it workflow-oriented and reference canonical docs instead of copying all rules.
5. Check current project facts against `AGENTS.md`, `gradle.properties`, and `docs/architecture.md` instead of hard-coding versions in this
   skill.
6. Verify file existence, links, and diff scope.

## Guardrails

- Do not modify Java source, Gradle files, `plugin.xml`, runtime resources, templates, icons, or message bundles for documentation-only
  tasks.
